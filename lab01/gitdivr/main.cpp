...
int main(int argc, char** argv) {
    ...
    while (q--) {
        ...
        for (int i = 0; i < c; i++) {
            ...
            if (sharedTail == nullptr) { sharedHead = node; }
            else { sharedTail->next = node; }
            sharedTail = node;
        }
        ...
        for (int i = 0; i < m; i++) {
            ...
            cin >> x;
            Node* node = new Node(x);
            if (onlyATail == nullptr) { onlyAHead = node; }
            else { onlyATail->next = node; }
            onlyATail = node;
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            cin >> x;
            Node* node = new Node(x);
            if (onlyBTail == nullptr) { onlyBHead = node; }
            else { onlyBTail->next = node; }
            onlyBTail = node;
        }

        // Point both branches at the same shared nodes, which forms the Y.
        if (onlyATail != nullptr) { onlyATail->next = sharedHead; }
        if (onlyBTail != nullptr) { onlyBTail->next = sharedHead; }
