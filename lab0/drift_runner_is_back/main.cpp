...
int main(int argc, char* argv[]) {
    ...
    for (unsigned n = step; n <= n_max; n += step) { sizes.push_back(n); }
    // this is to create sizes array which do not contain all the ints as sizes but only in multiples of step

    // here we have created ResultMap for each of the containers
    ...
    for (unsigned n : sizes) {
        std::cout << "vector " << n << " " << vec_r[n].insert_us << " " << vec_r[n].find_std_us << " "
                  << vec_r[n].find_member_us << "\n";
    }
    for (unsigned n : sizes) {
        std::cout << "list " << n << " " << list_r[n].insert_us << " " << list_r[n].find_std_us << " "
                  << list_r[n].find_member_us << "\n";
    }
    for (unsigned n : sizes) {
        std::cout << "set " << n << " " << set_r[n].insert_us << " " << set_r[n].find_std_us << " "
                  << set_r[n].find_member_us << "\n";
    }
    for (unsigned n : sizes) {
        std::cout << "unordered_set " << n << " " << uset_r[n].insert_us << " " << uset_r[n].find_std_us << " "
                  << uset_r[n].find_member_us << "\n";
    }
