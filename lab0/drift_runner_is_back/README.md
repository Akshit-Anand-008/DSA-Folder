...
This saves an image, `plot_data_plot.png`, with two graphs side by
side, and also prints the exact path it saved to. \
The two graphs are:
- **Insertion time vs. n**, one line per container.
- **Find time vs. n**, with a `std::find()` line for every container,
  plus a second, dashed `.find()` line for `set` and `unordered_set`.

Look at the *shape* of each line, not just its height - a flat line
means O(1), a slowly rising curve means O(log n), and a straight
diagonal line means O(n).

# Files to Edit [Do not edit any other files]

containers.cpp
tests/test1 : for this activity you can try playing around with the input values.
